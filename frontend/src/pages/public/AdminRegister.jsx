...
export default function AdminRegisterPage() {
    ...
    const registerHandler = async (e) => {
        ...
        try {
            setError('');
            const req = await axios.post('http://localhost:3000/admin/register', {
                name,
                email,
                password
            });
            ...
            <Card style={{ width: '100%', maxWidth: '420px', padding: '2.5rem', background: 'var(--card-bg)' }}>
                ...
                <div style={{ textAlign: "center", marginBottom: "2rem" }}>
                    <h2 style={{ fontSize: '2.5rem', fontWeight: '800' }}>
                        <span style={{ color: "var(--text-main)" }}>Admin Reg</span>
                ...
                <div style={{ marginTop: "1.5rem", textAlign: "center", fontSize: "0.95rem", fontWeight: '500' }}>
                    Already have an admin account? <span style={{ color: "var(--text-main)" }}>Go to</span> <Link to="/admin/login" style={{ color: "var(--primary)", textDecoration: 'none' }}>Login</Link>
                </div>
